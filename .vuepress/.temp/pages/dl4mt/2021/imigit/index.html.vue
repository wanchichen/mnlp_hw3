<template><div><p>Machine translation has helped people daily life, and is also an important research topic especially in computer science community.
...
<!-- more -->
<p>Below is an example of multimodal machine translation, which is try to caption the image in two language and use them as paired translation.
<img src="@source/dl4mt/2021/imigit/images/translation.png" alt="Multimodal Machine Translation">
...
<img src="@source/dl4mt/2021/imigit/images/MultimodalNMT.png" alt="Multimodal NMT"></p>
<h3 id="_1-1-challenge" tabindex="-1"><a class="header-anchor" href="#_1-1-challenge" aria-hidden="true">#</a> 1.1 Challenge</h3>
<p>Current state-of-the-art methods are built upon solely textual information, which is not consistent with how human try to implement the tasks. Most of the time, human would leverage some understanding or memory from visual information to help translation process.
...
But this is a really challenging task, given that the paired images of texts are scarce and synthesized images from textural input are of low quality.</p>
<h3 id="_1-2-motivation" tabindex="-1"><a class="header-anchor" href="#_1-2-motivation" aria-hidden="true">#</a> 1.2 Motivation</h3>
<p>Existing multi modal NMT need the paired image as the input, which limit its development, since such bilingual parallel corpora annotated by images are not always available.
And the cost of manual annotation is pretty high.
This paper try to avoid the need of paired image by synthesizing it, thus address the bottlenecks mentioned above.</p>
<h2 id="_2-methodology" tabindex="-1"><a class="header-anchor" href="#_2-methodology" aria-hidden="true">#</a> 2. Methodology</h2>
<p><strong>ImagiT</strong> is composed of roughly three key components, including imagination, visual representation module and common natural language translation model.
...
<h3 id="_2-1-problem-definition" tabindex="-1"><a class="header-anchor" href="#_2-1-problem-definition" aria-hidden="true">#</a> 2.1 Problem Definition</h3>
<p>The problem here is given a source language, we want to translate it into a target language in textual modal. In the meantime, instead of gathering paired images, we synthesize the imagination from textual input. Then the imagination is leveraged as the external knowledge, which contains some spatial correspondence to help further NMT task.</p>
<h3 id="_2-2-overview" tabindex="-1"><a class="header-anchor" href="#_2-2-overview" aria-hidden="true">#</a> 2.2 Overview</h3>
<p>ImagiT embodies the encoder-decoder structure for end-to-end machine translation. Between the encoder and the decoder, there is an imagination step to generate semanticequivalent visual representation. Technically, the model is composed of following modules: source text encoder, generative imagination network, image captioning, multimodal aggregation and decoder for translation.
<img src="@source/dl4mt/2021/imigit/images/architecture.png" alt="Overview"></p>
<h3 id="components" tabindex="-1"><a class="header-anchor" href="#components" aria-hidden="true">#</a> Components</h3>
<p>For source text encoder, they utilize the state-of-art Transformer-based machine translation framework.
...
To train the whole network end-to-end, they leverage adversarial training to alternatively train the generator and the discriminator.</p>
<h2 id="_3-experiments" tabindex="-1"><a class="header-anchor" href="#_3-experiments" aria-hidden="true">#</a> 3. Experiments</h2>
<h3 id="_3-1-datasets" tabindex="-1"><a class="header-anchor" href="#_3-1-datasets" aria-hidden="true">#</a> 3.1 Datasets</h3>
<p>They evaluate the proposed ImagiT model on two datasets, Multi30K and Ambiguous COCO. To show its ability to train with external out-of-domain datasets, they adopt MS COCO in the next analyzing section.
And the baseline is the conventional text-only Transformer.</p>
<h3 id="_3-2-configuration" tabindex="-1"><a class="header-anchor" href="#_3-2-configuration" aria-hidden="true">#</a> 3.2 Configuration</h3>
<p>For the imagination network, the noise vector’s dimension is 100, and the generated visual feature is 128 × 128. The upsampling and residual block in visual feature transformers consist of 3×3 stride 1 convolution, batch normalization, and ReLU activation. For detailed experimental settings, please refer to the paper.</p>
<h3 id="_3-1-results" tabindex="-1"><a class="header-anchor" href="#_3-1-results" aria-hidden="true">#</a> 3.1 Results</h3>
<p><img src="@source/dl4mt/2021/imigit/images/experiments.png" alt="Experiments">
As can be seen from above table, the main result from the Test2016, Test2017 for the En⇒De and En⇒Fr MNMT task is reported. The first category (Multimodal Neural Machine Translation Systems) collects the existing MNMT systems, which take both source sentences and paired images as input. The second category illustrates the systems that do not require images as input. Since our method falls into the second group, the baselines are the text-only Transformer and the aforementioned works.</p>
<h2 id="_4-conclusion-and-discussin" tabindex="-1"><a class="header-anchor" href="#_4-conclusion-and-discussin" aria-hidden="true">#</a> 4. Conclusion and Discussin</h2>
<p>Generally, this paper proposed a novel method to generate semantic-consistent visual representations for imagination-guided translation without any anootated images.
...
Future work might be combining with knowledge based imagination for more complex and abstract textual information.</p>
<h2 id="reference" tabindex="-1"><a class="header-anchor" href="#reference" aria-hidden="true">#</a> Reference</h2>
<ul>
<li>Quanyu Long, Mingxuan Wang, Lei Li. Generative Imagination Elevates Machine Translation. NAACL 2021. <a href="https://arxiv.org/abs/2009.09654" target="_blank" rel="noopener noreferrer">https://arxiv.org/abs/2009.09654<ExternalLinkIcon/></a></li>
</ul>
</div></template>
