<template><div><p>How to develop a translation model that can take both speech and text as input and translate to target language?
...
<!-- more -->
<p>Paper：<a href="https://arxiv.org/pdf/2105.03095.pdf" target="_blank" rel="noopener noreferrer">https://arxiv.org/pdf/2105.03095.pdf<ExternalLinkIcon/></a></p>
<p>Github: <a href="https://github.com/Glaciohound/Chimera-ST" target="_blank" rel="noopener noreferrer">https://github.com/Glaciohound/Chimera-ST<ExternalLinkIcon/></a></p>
<h2 id="introduction" tabindex="-1"><a class="header-anchor" href="#introduction" aria-hidden="true">#</a> Introduction</h2>
<p><img src="@source/dl4mt/2021/chimera/c1.png" alt="image1"></p>
<p>Although it seems difficult for normal people to acquire more than two languages, according
...
The challenge comes from the intrinsic difference of modality between audio and text.</p>
<p>A recent work Chimera from ByteDance AI Lab and UIUC aims to draw strengths from both modalities
...
to boost the ST performance.</p>
<p>There are two main advantages for including audio and text data together for training one ST model.
...
when bridging the modality gap properly.</p>
<p>Taking those benefits into consideration, the Chimera model showed significant improvements by unifying MT and ST
tasks on the benchmark ST datasets containing more than ten languages pairs.</p>
<p>Unlike previous translation models, Chimera has established a successful paradigm of
...
the pixel level information in images is accurately described, the audio is even noisier and leads to more challenges.</p>
<p>Chimera is designed for an end-to-end speech-to-text translation task. It has two advantages.
...
from the shared semantic space and thus paves a new way for augmenting training resources across modalities.</p>
<p>Chimera can be thought of as a multimodal in the field of speech translation in general.
...
The research data, codes and resources are also kindly published by the authors.</p>
<p>Next, we will introduce and analyze Chimera from three aspects: 1) the challenges of bridging the gap
between audio and text for speech translation; 2) the motivation and methods of Chimera; 3) the
performance and analysis of Chimera.</p>
<h2 id="challenges-in-speech-translation" tabindex="-1"><a class="header-anchor" href="#challenges-in-speech-translation" aria-hidden="true">#</a> Challenges in speech translation</h2>
<p>The current paradigm of artificial intelligence highly depends on training on a large dataset, and thus
...
speech and text and can serve as a new foundation in this area.</p>
<p><img src="@source/dl4mt/2021/chimera/c3.png" alt="image2"></p>
<p>In the above figure[2], the color represents relative contribution of unimodal visual (green), unimodal auditory
...
brain, serving as theoretical foundation of modality fusion.</p>
<h2 id="motivation-and-techniques-of-chimera" tabindex="-1"><a class="header-anchor" href="#motivation-and-techniques-of-chimera" aria-hidden="true">#</a> Motivation and Techniques of Chimera</h2>
<p><img src="@source/dl4mt/2021/chimera/c5.png" alt="image4"></p>
<p>​For language learners, a very interesting phenomenon is that they learn better by audio and text
...
of human children.</p>
<p><img src="@source/dl4mt/2021/chimera/c6.png" alt="image5"></p>
<p>The above figure shows the developmental milestones of human children learners. Children first learns
how to speak from their family and thus how to speak fluently, read and write formally at school.</p>
<p>All those evidence demonstrates that combining audio and text could help humans learn a new language.
A natural next step is to apply this idea in speech machine translation.</p>
<p>The design goal of Chimera is based on such considerations: design a general framework
...
in ST task.</p>
<h2 id="shared-semantic-memory" tabindex="-1"><a class="header-anchor" href="#shared-semantic-memory" aria-hidden="true">#</a> Shared semantic memory</h2>
<p><img src="@source/dl4mt/2021/chimera/c7.png" alt="image6">
...
The Decoder Module decodes semantic memory translation.</p>
<p>Since the encoder and decoder are actually standard modules based on transformer[5], which have been proven
...
<h2 id="dataset-and-preprocessing" tabindex="-1"><a class="header-anchor" href="#dataset-and-preprocessing" aria-hidden="true">#</a> Dataset and preprocessing</h2>
<p>Two datasets were used for conducting experiments to verify the effectiveness of Chimera.
...
They also incorporate data from WMT, OpenSubtitles and OPUS100 translation tasks as pretraining corpora.</p>
<p>In practice, speech input, the 16-bit raw wave sequences are normalized by a factor of 215 to the range of
...
attention heads.</p>
<p>Chimera contains around 165M parameters. The whole training process for one trial on 8 Nvidia Tesla-V100 GPUs
generally takes 20 –40 hours according to the translation direction.</p>
<h2 id="effectiveness-of-chimera" tabindex="-1"><a class="header-anchor" href="#effectiveness-of-chimera" aria-hidden="true">#</a> Effectiveness of Chimera</h2>
<p>In summary, Chimera has the following advantages:</p>
<ol>
<li>New state-of-the-art performance on all language pairs</li>
</ol>
<p>Even though they did not use Google Translate results on Augmented Librispeech as most baselines,
...
feature repeated lines, and address issues that are not covered in MuST-C public speeches.</p>
<ol start="2">
<li>Successfully share knowledge across tasks</li>
</ol>
<p>Additional trial findings corroborate their design of auxiliary tasks by demonstrating its
ability to acquire a well-structured shared semantic space as well as successfully exchange
learned knowledge between MT and ST.</p>
<p>Here are some representative experimental results:</p>
<h3 id="_1-benchmark-experiments" tabindex="-1"><a class="header-anchor" href="#_1-benchmark-experiments" aria-hidden="true">#</a> 1. Benchmark Experiments</h3>
<p>The below two tables demonstrate the main results on tst-COMMON subset on all 8 languages in MuST-C dataset
amd on LibriSpeech English-French dataset.</p>
<p><img src="@source/dl4mt/2021/chimera/c8.png" alt="image7"></p>
<p>Table 1: Main results on tst-COMMON subset on all 8 languages in MuST-C dataset.</p>
<p><img src="@source/dl4mt/2021/chimera/c9.png" alt="image8"></p>
<p>Table 2: Results on LibriSpeech English-French dataset.</p>
<h3 id="_2-visualizations" tabindex="-1"><a class="header-anchor" href="#_2-visualizations" aria-hidden="true">#</a> 2. Visualizations</h3>
<p><img src="@source/dl4mt/2021/chimera/c10.png" alt="image9"></p>
<p>Regardless of the input modality, the shared semantic projection is designed to extract only
the semantic categories of information required for decoding. To validate this hypothesis, a
2-dimensional PCA projection was performed in the semantic memories across different samples.</p>
<p>In the above figure, each colored cluster (circled out) represents a semantic memory element. A '.' corresponds to
...
distance of speech and text representations inside the same region.</p>
<p><img src="@source/dl4mt/2021/chimera/c11.png" alt="image9"></p>
<p>One randomly selected semantic memory subspace was analyzed by PCA to its related cluster to get
a better look at the structure of each semantic memory subspace.</p>
<p>The above figure is the visualization of one specific semantic memory with no different samples or
...
considering audio and text represent different modalities.</p>
<h2 id="summary" tabindex="-1"><a class="header-anchor" href="#summary" aria-hidden="true">#</a> Summary</h2>
<p>Going back to the polyglots, who successfully learn new languages through various kinds of interactions
...
convey common knowledge between these two tasks, paving the path for novel ways to supplement training materials across modalities.</p>
<p>In the future, we are looking forward to more advanced techniques to solve two additional problems: 1) how to
...
language barriers among countries and nations are broken, is waiting fo us.</p>
<h2 id="references" tabindex="-1"><a class="header-anchor" href="#references" aria-hidden="true">#</a> References</h2>
<p>[1] Han, Chi, Mingxuan Wang, Heng Ji, and Lei Li. &quot;Learning Shared Semantic Space for Speech-to-Text Translation.&quot; ACL 2021.</p>
<p>[2] Van Atteveldt, Nienke, Elia Formisano, Rainer Goebel, and Leo Blomert. &quot;Integration of letters and speech sounds in the human brain.&quot; Neuron 43, no. 2 (2004): 271-282.</p>
<p>[3] <a href="https://en.wikipedia.org/wiki/Ioannis_Ikonomou" target="_blank" rel="noopener noreferrer">https://en.wikipedia.org/wiki/Ioannis_Ikonomou<ExternalLinkIcon/></a></p>
<p>[4] Alexei Baevski, Yuhao Zhou, Abdelrahman Mohamed, and Michael Auli. 2020. wav2vec 2.0: A framework for self-supervised learning of speech representations. In Advances in Neural Information Processing Systems, volume 33, pages 12449–12460. Curran Associates, Inc.</p>
<p>[5] Vaswani, Ashish, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Łukasz Kaiser, and Illia Polosukhin. &quot;Attention is all you need.&quot; In Advances in neural information processing systems, pp. 5998-6008. 2017.</p>
</div></template>
